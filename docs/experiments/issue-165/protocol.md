...
Issue #165 prospectively defines exactly these terminal determinations:

* `RETROSPECTIVE_DECISION_RELEVANCE_SUPPORTED`
* `NO_INCREMENTAL_DECISION_RELEVANCE`
* `MISLEADING_STRUCTURAL_OUTPUT`
* `INDETERMINATE`
* `EXPERIMENT_INVALID`
* `BLOCKED_BY_REPOSITORY_OR_HISTORY`

For this execution, the observed evidence maps to
`NO_INCREMENTAL_DECISION_RELEVANCE`: execution completed and the material
structural facts recovered by SYNAPSE were already present in the ordinary
control or obtainable through the simpler direct inspection.

## Hindsight-protection integrity note

Issue #165 also requires preservation, before later-history reveal, of:

1. control memo;
2. raw SYNAPSE output;
3. analyst interpretation;
4. candidate consequential findings;
5. confidence ratings;
6. artifact hashes.

This experiment directory preserves the control memo, raw artifact, and hashes.
It does **not** contain a separately frozen pre-reveal analyst-interpretation /
candidate-findings / confidence artifact. That omission is not repaired or
reconstructed after the fact. It is retained as a protocol-integrity limitation
on this pilot.
