...
**Terminal classification: NOT SUPPORTED.**
...
control reported.  Under the prospective rubric, reproducible restatement is
not additional value, so the result is NOT SUPPORTED and is retained without
...
Issue 165 can legitimately close as an executed negative pilot: the procedure
completed, its evidence is replayable, and the terminal classification is
bounded.  It must not close as validation of external adoption or superior
decision recovery.  No directly related stale pilot artifacts existed before
this work; this directory is the single canonical experiment record.
